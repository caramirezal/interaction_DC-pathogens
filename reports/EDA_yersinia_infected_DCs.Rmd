...
BysvsMock.cdp.24hpi.dotplot <- DotPlot(
  ...
  labs(x='', y='') +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  ggtitle('CDP - 24HPI - Bystander vs Mock')
...
# Evaluation of CDP and MDP potential along trajectories

```{r, cdp_mdp_signatures}
myeloid.sign <- read_xlsx('data/gene signatures of DCs and monocytes.xlsx')

## Processing gene names
dc.signature <- myeloid.sign$DCs[2:nrow(myeloid.sign)]
dc.signature <- sapply(dc.signature, 
                           function(x) 
                             paste0(toupper(substr(x, 1, 1)),
                                    substr(x, 2, nchar(x))))
mono.signature <- myeloid.sign$monocytes[2:nrow(myeloid.sign)]
mono.signature <- mono.signature[!is.na(mono.signature)]
mono.signature <- sapply(mono.signature, 
                           function(x) 
                             paste0(toupper(substr(x, 1, 1)),
                                    substr(x, 2, nchar(x))))
...
## Getting cells in the diffusion maps
dm.cells <- lapply(dif.maps, rownames) %>%
               unlist() %>%
                 unname()
seurat.dm <- subset(sc.seurat, well %in% dm.cells)
seurat.dm <- AddModuleScore(seurat.dm, 
                            features = list(dendritic=dc.signature,
                                            monocyte=mono.signature), 
                            name = c('dendritic_signature', 
                                     'monocyte_signature'), 
                            nbin = 5)

dc.sign.vln <- VlnPlot(seurat.dm, 
        group.by = 'Bystander', 
        features = 'dendritic_signature1') +
        NoLegend() + xlab('') +
        ggtitle('Dendritic Signature')

mono.sign.vln <- VlnPlot(seurat.dm, 
        group.by = 'Bystander', 
        features = 'monocyte_signature2') +
       NoLegend() + xlab('') +
      ggtitle('Monocyte Signature')

#gridExtra::grid.arrange(dc.sign.vln, 
#                        mono.sign.vln, ncol=2)
```

### Dendritic signature


```{r, scores_in_diffusion_dendritic, fig.width=12, fig.height=8}
## Adding scores to diffusion maps
dif.maps <- lapply(dif.maps, 
          function(df) {
            df$'dendritic_signature' <- seurat.dm@meta.data[rownames(df),
                                                           'dendritic_signature1']
           df$'monocyte_signature' <- seurat.dm@meta.data[rownames(df),
                                                           'monocyte_signature2'] 
           df
})

cdp.1hp.dc <- dif.maps[1][[1]] %>%
  ggplot(aes(x=DC1, y=-DC2,
             colour=`dendritic_signature`)) +
      geom_point(size=3) +
      scale_color_viridis() +
      theme_classic() +
      ggtitle(names(dif.maps)[1])

mdp.1hp.dc <- dif.maps[2][[1]] %>%
  ggplot(aes(x=DC1, y=DC2,
             colour=`dendritic_signature`)) +
      geom_point(size=3) +
      scale_color_viridis() +
      theme_classic() +
      ggtitle(names(dif.maps)[2])

cdp.24hp.dc <- dif.maps[3][[1]] %>%
  ggplot(aes(x=DC1, y=DC2,
             colour=`dendritic_signature`)) +
      geom_point(size=3) +
      scale_color_viridis() +
      theme_classic() +
      ggtitle(names(dif.maps)[3])

mdp.24hp.dc <- dif.maps[4][[1]] %>%
  ggplot(aes(x=DC1, y=-DC2,
             colour=`dendritic_signature`)) +
      geom_point(size=3) +
      scale_color_viridis() +
      theme_classic() +
      ggtitle(names(dif.maps)[4])
...
```{r, scores_in_diffusion_monocyte, fig.width=12, fig.height=8}

cdp.1hp.mono <- dif.maps[1][[1]] %>%
  ggplot(aes(x=DC1, y=-DC2,
             colour=`monocyte_signature`)) +
      geom_point(size=3) +
      scale_color_viridis() +
      theme_classic() +
      ggtitle(names(dif.maps)[1])

mdp.1hp.mono <- dif.maps[2][[1]] %>%
  ggplot(aes(x=DC1, y=DC2,
             colour=`monocyte_signature`)) +
      geom_point(size=3) +
      scale_color_viridis() +
...
prog.byst.196 <- sc.seurat %>%
  ...
  mutate(selected_gene=(toupper(gene) %in% toupper(selected_genes))) %>%
  filter(!grepl('^Gm', gene)) %>%
  mutate(highlight=ifelse( ( abs(avg_log2FC) > 1.8  & p_val < 0.005),
                           TRUE, FALSE) ) %>%
  mutate(label=ifelse(highlight==TRUE | 
                        selected_gene ==TRUE, gene, ''))
...
```{r, cdpvsmdp_dotplots, fig.width=14, fig.height=12}
## Getting top markers
cdpVsmdp <- list(prog.inf.193=prog.inf.193,
                 prog.byst.193=prog.byst.193,
                 prog.mock.193=prog.mock.193,
                 prog.inf.196=prog.inf.196,
                 prog.byst.196=prog.byst.196,
                 prog.mock.196=prog.mock.196)
cdpVsmdp <- lapply(cdpVsmdp, 
                   function(x) 
                     filter(x, p_val < 0.005) %>%
                     arrange(desc(avg_log2FC)))

## CDP vs MDPs - 1HPI - Infected 
inf.1hpi.dotplot <- subset(sc.seurat, plate_number=='P193') %>%
  subset(Bystander == 'Infected') %>%
  DotPlot( 
  features = unique(c(head(cdpVsmdp$prog.inf.193$gene, 10),
               tail(cdpVsmdp$prog.inf.193$gene, 10)))
) + coord_flip() +
  scale_color_viridis() +
  labs(x='', y='') +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
## CDP vs MDPs - 1HPI - Bystander
byst.1hpi.dotplot <- subset(sc.seurat, 
                            plate_number=='P193') %>%
  subset(Bystander == 'Bystander') %>%
  DotPlot( 
  features = unique(c(head(cdpVsmdp$prog.byst.193$gene, 10),
               tail(cdpVsmdp$prog.byst.193$gene, 10)))
) + coord_flip() +
  scale_color_viridis() +
  labs(x='', y='') +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
## CDP vs MDPs - 1HPI - Mock
mock.1hpi.dotplot <- subset(sc.seurat, 
                            plate_number=='P193') %>%
  subset(Bystander == 'Mock-Infected') %>%
  DotPlot( 
  features = unique(c(head(cdpVsmdp$prog.mock.193$gene, 10),
               tail(cdpVsmdp$prog.mock.193$gene, 10)))
) + coord_flip() +
  scale_color_viridis() +
  labs(x='', y='') +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  ggtitle('CDP vs MDP - 1HPI - Mock')


##################################################################

## CDP vs MDPs - 24HPI - Infected 
inf.24hpi.dotplot <- subset(sc.seurat, plate_number=='P196') %>%
  subset(Bystander == 'Infected') %>%
  DotPlot( 
  features = unique(c(head(cdpVsmdp$prog.inf.196$gene, 10),
               tail(cdpVsmdp$prog.inf.196$gene, 10)))
) + coord_flip() +
  scale_color_viridis() +
  labs(x='', y='') +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
## CDP vs MDPs - 24HPI - Bystander
byst.24hpi.dotplot <- subset(sc.seurat, plate_number=='P196') %>%
  subset(Bystander == 'Bystander') %>%
  DotPlot( 
  features = unique(c(head(cdpVsmdp$prog.byst.196$gene, 10),
               tail(cdpVsmdp$prog.byst.196$gene, 10)))
) + coord_flip() +
  scale_color_viridis() +
  labs(x='', y='') +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
## CDP vs MDPs - 24HPI - Mock
mock.24hpi.dotplot <- subset(sc.seurat, plate_number=='P196') %>%
  subset(Bystander == 'Mock-Infected') %>%
  DotPlot( 
  features = unique(c(head(cdpVsmdp$prog.mock.196$gene, 10),
               tail(cdpVsmdp$prog.mock.196$gene, 10)))
) + coord_flip() +
  scale_color_viridis() +
  labs(x='', y='') +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  ggtitle('CDP vs MDP - 24HPI - Mock')
...
```{r, saving_results}
## degs comparing infection status at 1 hpi
write_xlsx(x = list('infvsBys.cdp.deg.1hpi'=infvsBys.cdp.deg,
             'infvsMock.cdp.deg.1hpi'=infvsMock.cdp.deg, 
             'bysvsMock.cdp.deg.1hpi'=bysvsMock.cdp.deg,
             'infvsBys.mdp.deg.1hpi'=infvsBys.mdp.deg,
             'infvsMock.mdp.deg.1hpi'=infvsMock.mdp.deg,
             'bysvsMock.mdp.deg.1hpi'=bysvsMock.mdp.deg), 
           path = 'analysis/degs.infection.status.1hpi.xlsx'
)

## degs comparing infection status at 24 hpi
write_xlsx(x = list('infvsBys.cdp.deg.24hpi'=infvsBys.cdp.deg.24,
             'infvsMock.cdp.deg.24hpi'=infvsMock.cdp.deg.24, 
             'bysvsMock.cdp.deg.24hpi'=bysvsMock.cdp.deg.24,
             'infvsBys.mdp.deg.24hpi'=infvsBys.mdp.deg.24,
             'infvsMock.mdp.deg.24hpi'=infvsMock.mdp.deg.24,
             'bysvsMock.mdp.deg.24hpi'=bysvsMock.mdp.deg.24), 
           path = 'analysis/degs.infection.status.24hpi.xlsx'
)

## degs comparing infection status at 24 hpi
write_xlsx(x = list('cdpVsmdp.mock.1hpi'=prog.mock.193,
             'cdpVsmdp.byst.1hpi'=prog.byst.193, 
             'cdpVsmdp.inf.1hpi'=prog.inf.193,
             'cdpVsmdp.mock.24hpi'=prog.mock.196,
             'cdpVsmdp.byst.24hpi'=prog.byst.196,
             'cdpVsmdp.inf.24hpi'=prog.inf.196), 
           path = 'analysis/degs.CDPVsMDP.xlsx'
)
